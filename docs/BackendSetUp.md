...
**Security testing:** dependency audits, checklist, OWASP mapping.
...
Runs `npm audit` on `backend`, `patient-app`, `caregiver-dashboard`, and the root package. Treat **high/critical** advisories as release blockers unless documented with mitigation; use `npm audit fix` where safe.
...
Messages, push (VAPID), email, WebRTC signalling, and full CRUD on every caregiver route are **not** part of this suite; covering them would need extra fixtures/mocks or E2E/browser tests. The suite above is intentionally **targeted API regression coverage** for auth, access control, reminders list, games persistence, and location pause behaviour. Broader security review uses **section 4.5** (dependency audit) plus manual review of auth, transport, and deployment configuration.
